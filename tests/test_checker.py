"""Checker Unit 테스트 (TDD).

Assembler(Tokenizer/Parser)가 아직 없으므로 AST(list[Stmt])를 직접 구성해서
Checker의 입력으로 준다. 테스트 스크립트.md 기준:
...
def test_modifying_outer_variable_without_redeclaration_is_allowed():
    ...
    stmts = [
        ...
        BlockStmt([
            ExpressionStmt(AssignExpr(ident("count"), VariableExpr(ident("count")))),
        ]),
    ]
    Checker(stmts).check()
...
def test_for_loop_variable_scope_is_allowed():
    ...
    stmts = [
        ForStmt(
            ...
            condition=BinaryExpr(VariableExpr(ident("j")), Token(TokenType.LESS, "<"), literal(3.0)),
            increment=AssignExpr(
                ident("j"),
                BinaryExpr(VariableExpr(ident("j")), Token(TokenType.PLUS, "+"), literal(1.0)),
            ),
