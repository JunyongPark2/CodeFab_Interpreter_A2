...
- 변수 선언은 `var`, 값 변경은  `=`. 입니다. (`var x = 1; x = 2;`)
- `if (조건) { ... } else { ... }` — 조건이 참이면 위 블록, 아니면 아래 블록이 실행됩니다.
-  `for (시작; 조건; 증감) { ... }` 을 지원합니다.
- `{ }` 블록은 독립된 스코프입니다. 블록 안에서 같은 이름으로 `var` 재선언하면 그 블록
  안에서만 새 변수로 취급됩니다. 블록을 나가면 원래 바깥 변수가 그대로 유지됩니다.
